 ...
 * Added a new member function for the `Reconstructor` class which can read in a whole
 ...
 directory in which the saved states are.
 ...
 * Fixed the singular system error which was caused rarely when the sequence of
 random numbers generated by the pcg32 algorithm contained the same number twice.
